...
const safeExternalUrl=value=>{try{const u=new URL(String(value||'').trim());return u.protocol==='https:'?u.href:''}catch{return''}};
...
function clearStoryForm(){editingId=null;if(typeof storyTitleInput!=='undefined')storyTitleInput.value='';if(typeof storySummaryInput!=='undefined')storySummaryInput.value='';if(typeof storyUrlInput!=='undefined')storyUrlInput.value='';if(typeof storyBodyInput!=='undefined')storyBodyInput.value='';if(typeof storyDateInput!=='undefined')storyDateInput.value=localNow();if(typeof storyFormHeader!=='undefined')storyFormHeader.textContent='Manage StoryBoard';if(typeof storySaveBtn!=='undefined')storySaveBtn.textContent='Publish Story';if(typeof storyCancelBtn!=='undefined')storyCancelBtn.style.display='none'}
async function saveStory(){
 const u=auth.currentUser;if(!u)return alert('Please sign in first.');
 const title=storyTitleInput.value.trim(),summary=storySummaryInput.value.trim(),body=storyBodyInput.value,date=storyDateInput.value||localNow(),titleLower=normTitle(title),rawUrl=storyUrlInput.value.trim(),externalUrl=safeExternalUrl(rawUrl);
 if(!title)return alert('Please enter a story title.');if(rawUrl&&!externalUrl)return alert('Please enter a valid HTTPS story URL.');if(externalUrl&&!summary)return alert('Please add a summary for the story card.');if(!externalUrl&&!body.trim())return alert('Please write the story or include an external URL.');if(body.length>50000)return alert('Story is too long. Maximum is 50,000 characters.');
 const data={title,titleLower,summary,externalUrl,body,date};
 try{if(editingId)await updateDoc(doc(db,'users',u.uid,'stories',editingId),data);else await addDoc(collection(db,'users',u.uid,'stories'),{...data,position:Date.now()});clearStoryForm();await loadAdminStories(u.uid);if(publicStoryOwner===u.uid)await resetPublicStories()}catch(e){console.error(e);alert('Could not save story. Check Firestore rules.')}
}
async function editStory(id){const s=stories.find(x=>x._id===id);if(!s)return;editingId=id;storyTitleInput.value=s.title||'';storySummaryInput.value=s.summary||'';storyUrlInput.value=s.externalUrl||'';storyBodyInput.value=s.body||'';storyDateInput.value=(s.date||'').slice(0,16);storyFormHeader.textContent='Edit Story';storySaveBtn.textContent='Update Story';storyCancelBtn.style.display='block';storyTitleInput.scrollIntoView({behavior:'smooth',block:'center'})}
